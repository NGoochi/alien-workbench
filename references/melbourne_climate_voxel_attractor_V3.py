#! python 2
"""
Melbourne Climate Voxel Attractor — Eto GUI  (V3 mesh modes)
=============================================================
...
def _extract_mesh_density(bound_mesh, min_pt, step_x, step_y, step_z,
                          grid_count, eff_z):
    """Extract per-cell original density from mesh vertex colors.
    Returns dict of (ci,cj,ck) -> density (0-1).
    If no vertex colors, uses vertex count per cell as density proxy."""
    cell_vals = {}    # (ci,cj,ck) -> list of color samples
    cell_counts = {}  # (ci,cj,ck) -> vertex count (for no-color fallback)
    verts = bound_mesh.Vertices
    colors = bound_mesh.VertexColors
    has_colors = colors is not None and colors.Count == verts.Count

    # Check if colors are all the same (uniform gray mesh)
    all_same = False
    if has_colors and colors.Count > 1:
        c0 = colors[0]
        all_same = True
        for ci_check in range(min(colors.Count, 20)):
            cc = colors[ci_check]
            if cc.R != c0.R or cc.G != c0.G or cc.B != c0.B:
                all_same = False
                break
    if all_same:
        has_colors = False  # treat uniform colors as no colors
    ...
    for vi in range(verts.Count):
        ...
        if 0 <= ci < grid_count and 0 <= cj < grid_count and 0 <= ck < eff_z:
            ...
            # Always count vertices per cell
    ...
    if has_colors:
        ...
        # Use vertex count as density proxy (normalised to 0-1)
        if cell_counts:
    ...
    while ix < grid_count:
        ...
        while iy < grid_count:
            ...
            while iz < eff_z:
                ...
                # Mesh modulate mode: multiply climate noise by original density
...
class AttractorGUI(forms.Dialog[System.String]):
    def __init__(self, profiles):
        ...
        self.mesh_mode_combo.DataStore = [
            "Replace with Climate",
            "Modulate Original",
        ]
        ...
        self.sl_grid = forms.Slider(MinValue=5, MaxValue=80, Value=15)
...
def main():
    ...
    while True:
        ...
        if result == "pick_brep":
            objs = rs.GetObjects("Select site geometry (Brep/Mesh, multiple OK)",
            ...
            if objs:
                ...
                for obj in objs:
                    ...
                    if geom:
                        ...
                        if m:
                            meshes.append(m)
                # Prefer mesh path if any meshes selected
                if meshes:
                    if len(meshes) == 1:
                        combined = meshes[0]
                    else:
                        ...
                        for m in meshes:
                            combined.Append(m)
                    # Also convert any breps to mesh and append
                    for b in breps:
                        mp = rg.MeshingParameters.Default
                        bm_list = rg.Mesh.CreateFromBrep(b, mp)
                        if bm_list:
                            for bm in bm_list:
                                combined.Append(bm)
                    dialog.bound_mesh = combined
                    print("Combined %d meshes + %d breps -> %d vertices" % (
                        len(meshes), len(breps), combined.Vertices.Count))
                elif breps:
                    if len(breps) == 1:
